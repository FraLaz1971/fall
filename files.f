...
C HP: HEALTH INSURANCE PERCENTAGE
        ...
        INTEGER NBH(NEMPL),STANU2(NEMPL),STANUM(NEMPL),WTWH(NEMPL)
     &,WTWM(NEMPL)
...
C UFILE is the unsorted data file
     ...
     &,HEAINS(NEMPL),NETPAY(NEMPL),TAX(NEMPL),WWH(NEMPL)
...
C GROSS PAY FIXED LIMIT FOR HEALTH INSURANCE IS 200 $
        ...
        CALL CAPARO(NEMPL,WWH,WTWH,WTWM,STANU2,STANUM,ENAME,BASRAT,
     &HIGRAT,NBH,GROPAY,TAX,HEAINS,NETPAY)
      END
...
C FROM SMALLER TO GREATER STAFF NUMBER), COMPUTE GROSS WEEKLY PAY
      SUBROUTINE CAPARO(N,WWH,WTWH,WTWM,STANU2,STANUM,ENAME,BASRAT,
     &HIGRAT,NBH,GROPAY,TAX,HEAINS,NETPAY)
        IMPLICIT NONE
