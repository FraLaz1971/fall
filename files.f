...
C WPFNAM: WEEKLY PAY FILE NAME (ONE FOR WEEK)
     ...
     &,HEAINS(N),NETPAY(N)
        ...
        CHARACTER*64 EFNAM,WPFNM1,WPFNM2
        LOGICAL DEBUG
        COMMON /CONST/TFA,HFL,TP,HP
...
C OPEN FIRST WEEKLY PAY DATA FILE
     ...
     &'bas.rate ','hi.rate ','#bas.h.'
        PRINT *,'name            ','GROPAY    ','TAX','  HEAINS ',
     &'NETPAY'
C FOR EACH EMPLOYEE
        DO 10,I=1,MAXEMP
          READ(11,100,END=80,ERR=9100)STANUM(I),ENAME(I),BASRAT(I)
     &,HIGRAT(I),NBH(I)
          READ(12,200,END=80,ERR=9200)STANU2(I),WTWH(I),WTWM(I)
...
C NETPAY(I)=GROPAY(I)-(TAX(I)-HEAINS(I))
            ...
            PRINT 120,ENAME(I),GROPAY(I),TAX(I),HEAINS(I),NETPAY(I)
          ELSE
            GOTO 9000
          END IF
10      CONTINUE
80      CLOSE(11)
        PRINT *,'THERE ARE ',I-1,'EMPLOYEES'
        GOTO 9999
100   FORMAT(I3,1X,A16,2(1X,F6.2),1X,I2)
110   FORMAT(I2,1X,I3,1X,A16,2(1X,F6.2),1X,I2)
120   FORMAT(A16,4(1X,F6.2))
200     FORMAT(I3,1X,I2,1X,I2)
9000    PRINT *,'ERROR IN INPUT FILES DETECTED: EXITING'
        GOTO 9999
9100    PRINT *,'ERROR READING FILE',EFNAM
        GOTO 9999
9200    PRINT *,'ERROR READING FILE',WPFNM1
        GOTO 9999
9300    PRINT *,'ERROR READING FILE',WPFNM2
9999    RETURN
      END
